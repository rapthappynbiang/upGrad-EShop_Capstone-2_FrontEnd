           ...
           <div className="content">
              {products.map(product=>(
                  <Card id={`card-no-${product.productId}`} style={cardItem}>
                      {window.sessionStorage.role === 'admin' && (()=>{
                          return(
                              <CardContent style={{width:'100%', display: 'flex', justifyContent: 'space-around'}}>
                                  <Button variant="contained" style={{backgroundColor: 'grey'}} onClick={()=>{editProduct(product.productId)}}>Edit</Button> 
                                  <Button variant="contained" style={{backgroundColor: 'red', color: 'white'}} onClick={()=>{deleteProduct(product.productId)}}>Delete</Button>
                              </CardContent>
                          )
                      })()}
                      <CardContent>
                     ...
                     <CardContent>
                         <Typography><em>{product.name}</em></Typography>
                     </CardContent>
                     <CardContent>
