...
import {Stepper, Step, StepLabel, Typography, Button, Card, CardContent, CardHeader, Select, InputLabel, MenuItem, FormControl, Input, FormHelperText} from '@material-ui/core';
import './Order.css';
...
function Order(props){
    ...
    const [quantity, setQuantity] = useState(Number(history.location.search.split('=')[1])); 
    ...
    const addAddress = ()=>{
        if(nameRequired !== "" || streetRequired !== '' || 
        cityRequired !== '' || stateRequired !== "" ||
         contactNumberRequired !== ""  || zipCodeRequired !== "" ){
                return;
        }
        ...
        loadData(`${history.baseURL}/address`, 'post', data, 
    ...
    const handleConfirmOrder =()=>{
        ...
        loadData(`${history.baseURL}/orders`, 'post', data,
         {"Authorization": `Bearer ${window.sessionStorage.getItem('x-auth-token')}`}, null)
         .then(response=>{
    ...
    useEffect(()=>{
        ...
        loadData(`${history.baseURL}/address`, 'get', null, 
        {'Authorization': `Bearer ${window.sessionStorage.getItem('x-auth-token')}`},
        null)
        .then(response=>{
                ...
                </div>
                {
                     ...
                     activeStep === 0 && (()=>{
                       ...
                       <div className="container details-container">
                          ...
                          <div>
                          <label htmlFor="quantity-input"><b>Quantity: </b></label>
                           <input id="quantity-input" value={quantity} type="number"
                            placeholder="QtY" className="quantity-input" 
                            onChange={(event)=>{setQuantityFn(event.target.value)}}
                            min={1}
                            max={product.availableItems}
                            />
                          </div>
                           <hr style={{fontSize: '2px', width: '100%'}}/>
                           <Typography variant="h6"><b>About this item</b></Typography>
                   ...
                   { //shipping address component step 2
                            ...
                            <div className="step-content" style={{display: 'block'}}>
                                <Card className="order-card">
                                  {addresses.length !== 0 ? (()=>{
                                      return (
                                       <CardContent>
                                           <InputLabel id="select-address-label">Select Shipping Address</InputLabel>
                                           <Select
                                           ...
                                           {addresses.map(address=>(
                                               <MenuItem key={address._id} value={address}>{address.name + " " + address.street+" "+address.landmark+" "+address.city+ " "+address.state+" Zip-code: "+address.zipCode+" contact:"+address.contactNumber}</MenuItem>
                                           ))}
                                           </Select>
                                      </CardContent>);
                               ...
                               <Card className='order-card'>
                                   <CardContent className="form-container">
                                       ...
                                       <FormControl required className="form">
                                           ...
                                           />
                                           <FormHelperText>
                                               <Typography style={{color: 'red'}}>{nameRequired}</Typography>
                                           </FormHelperText>
                                       </FormControl>
                                       ...
                                       <FormControl required>
                                           ...
                                           />
                                           <FormHelperText>
                                               <Typography style={{color: 'red'}}>{streetRequired}</Typography>
                                           </FormHelperText>
                                       </FormControl>
                                       ...
                                       <FormControl>
                                           ...
                                           />
                                           <FormHelperText>
                                               <Typography style={{color: 'red'}}>{cityRequired}</Typography>
                                           </FormHelperText>
                                       </FormControl>
                                       ...
                                       <FormControl>
                                           ...
                                           />
                                           <FormHelperText>
                                               <Typography style={{color: 'red'}}>{stateRequired}</Typography>
                                           </FormHelperText>
                                       </FormControl>
                                       ...
                                       <FormControl>
                                           ...
                                           />
                                           <FormHelperText>
                                               <Typography style={{color: 'red'}}>{contactNumberRequired}</Typography>
                                           </FormHelperText>
                                       </FormControl>
                                       ...
                                       <FormControl>
                                           ...
                                           />
                                           <FormHelperText>
                                               <Typography style={{color: 'red'}}>{zipCodeRequired}</Typography>
                                           </FormHelperText>
                                       </FormControl>
                                   </CardContent>
                                       <CardContent style={{justifyContent: 'end', display: 'flex'}}>
                    ...
                    { activeStep === 2 && (()=>{
                            ...
                            <Card className="order-card order-detail-card">
                                ...
                                <hr style={{fontSize: '2px', width: '100%'}}/>
                                <CardContent>
                                    <Typography><b>Name:</b> {selectedAddress.name}</Typography>
                                </CardContent>
                                <CardContent>
                                    <Typography><b>Shipping address:</b> {`${selectedAddress.street} ${selectedAddress.landmark} ${selectedAddress.state} ${selectedAddress.city} Zip-code-${selectedAddress.zipCode}`}</Typography>
                                </CardContent>
                                <CardContent>
                                    <Typography><b>Contact Number: </b>{selectedAddress.contactNumber}</Typography>
                                </CardContent>
                                <CardContent>
                                    <Typography><b>Product:</b> {product.name}</Typography>
                                </CardContent>
                                <CardContent>
                                    <Typography><b>Manufacturer:</b> {product.manufacturer}</Typography>
                                </CardContent>
                                ...
                                <CardContent>
                                    <Typography><b>Price/item:</b> {product.price}</Typography>
                                </CardContent>
                                <hr style={{fontSize: '2px', width: '100%'}}/>
                                <CardContent>
                                    <Typography variant="h6"><b>Total: M.R.P-</b> {quantity*product.price}</Typography>
                                </CardContent>
                            </Card>  
                        </div>);
                        })()}  
                        {activeStep === steps.length && (()=>{
                            return (<div className = "success-container">
                                        <Card className="card">
                                            <CardContent style={{display: 'flex', justifyContent: 'Center', alignItems: 'center', height: '30%'}}>
                                                <Typography variant="h5"><em>{SuccessfullMessage}</em></Typography>
