...
function Header(props){
    ...
    const onCategoryChange=(event)=>{  
        ...
        var url=props.baseURL+`/products?category=${event.target.value}&sortBy=${sortBy===""?"":sortBy.split(" ")[0]}&direction=${sortBy===""?"DESC":sortBy.split(" ")[1]}`;
    ...
    const onSearchClick=()=>{
        var url=props.baseURL+`/products?category=${category}&name=${productname}&sortBy=${sortBy===""?"":sortBy.split(" ")[0]}&direction=${sortBy===""?"DESC":sortBy.split(" ")[1]}`;
        loadData(url,'get', null, null, null)
        .then(response=>{
            props.setProducts(response.data);
        })
    ...
    return (
        <AppBar style={appBar}>
            ...
            {(()=>{
                ...
                if(!window.sessionStorage.getItem('isLoggedIn') || window.sessionStorage.getItem('isLoggedIn') === undefined){
                    return(
                        <div className="header-links" style={{justifyContent: 'flex-end'}}> 
                    ...
                    return(
                        <div className="header-links">
                            ...
                            <FormControl id="form-1" variant="filled" style={filterFields}>
                                <InputLabel id="selsect-category-label" shrink={true} style={{color: 'black'}}>Category</InputLabel>
                                <Select 
                            ...
                            <FormControl id="form-2" variant="filled" style={filterFields}>
                                <InputLabel id="select-sort-label" shrink={true} style={{color: 'black'}}>Sort By</InputLabel>
                                <Select 
                            ...
                            <FormControl id="form-3" style={{width: '40%'}}> 
                               <input id="productName" placeholder="Product name" value={productname} onChange={onProductNameChange} className="search-bar"/>
                            </FormControl>
                            <Search className="searchIcon" onClick={()=>{onSearchClick()}} />
                            </div>
                            

                            <ul className="links-container" style={{width: window.sessionStorage.getItem('role')==="admin"?'20%':'15%'}}>
                                <li><Link to='/' className="links">Home</Link></li>
                                {window.sessionStorage.getItem('role') === "admin" && (()=>{
                                        return  <li><Link to='/addproduct' className="links">Add Product</Link></li>
                                })()}
